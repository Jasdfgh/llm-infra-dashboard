#!/bin/bash
# ═══════════════════════════════════════════════════════════
# ops_cheatsheet.sh — daily operations quick reference
# This file is for reference only. Copy the commands you need.
# ═══════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════
# Manual sync — anyone can trigger manually
# ══════════════════════════════════════════════════════════
...
# Note: --since filters on updated_at >= since, not created_at.
# An issue created in 2023 will be returned if someone commented on it after 4/1.

# ── No label filter (fetch all issues/PRs) ──
# Note: large repos may have tens of thousands of items; add --since to limit the time range
...
# ══════════════════════════════════════════════════════════
# Scheduled sync — systemd timer (already configured, runs automatically)
# ══════════════════════════════════════════════════════════
...
# 3. Run quickstart for classification (replace your_classifier() with your LLM classifier)
...
#
# You can also specify a historical version (commit/tag):
#   get_file_contents(owner="vllm-project", repo="vllm",
#       path="vllm/v1/attention/backends/mla/rocm_aiter_mla.py",
#       branch="v0.8.0")  # a specific tag
#
# Or via CLI:
...
# 3. Restart MCP service to apply config: systemctl --user restart signals-sync-mcp.service
